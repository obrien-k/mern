...
export const useAllCommunities = () => {
  ...
  const { communities, communityGroups, error } = useSelector(
  ...
  useEffect(() => {
    if (communities && communities.length > 0) {
      ...
      batch(() => {
        const groupPromises = communities.map((community) =>
    ...
    return communities.map((community) => {
      ...
      return {
        ...
        groups: groups.map((group) => {
          const year =
            group.isEdition && group.edition ? group.edition.year : undefined;
          const contributors = Array.isArray(group.contributors)
            ? group.contributors.length
            : group.contributors;
          const consumers = Array.isArray(group.consumers)
